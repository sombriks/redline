...
export const findMovimentacao = async (
	id, // TODO check account and user too
) => knex("movimentacao").where({ id }).first();

export const listMovimentacaoByUsuario = async (params) => {
	const { usuario_id = -1 } = params;

	return prepareMovimentacaoQuery(params).whereIn(
		"conta_id",
...
const prepareMovimentacaoQuery = (params, whereParams = {}) => {
	const {
		// TODO aplicar joi ? https://joi.dev/api/?v=17.9.1#general-usage
		q = "",
	...
	if (
		...
		query = query.whereBetween("vencimento", [
...
export const updateMovimentacao = async ({ id = -1, movimentacao }) => {
	movimentacao.id = id;
	return knex("movimentacao").update(movimentacao).where({ id });
};

export const removeMovimentacao = async (id = -1) =>
	knex("movimentacao").where({ id }).del();
...
export const pagamento = async ({
	...
	const transferencias = await transferencia({
		origem,
		destino,
		categoria,
		valor,
	...
	await knex("movimentacao")
		.update({ efetivada: vencimento })
		.whereIn("id", movimentacoes_id);
...
export const uploadMovimentacoes = async ({ id, header, lines }) => {
	...
	if (Object.keys(headerMap).some((k) => headerMap[k] === -1))
		throw new Error(
			"Unexpected header. Accepted header is " + Object.keys(headerMap),
		);
	const accountMap = {};
	const categoryMap = {};
	...
	for (let line of lines) {
		...
		try {
			let tipoMovimentacao =
				"entrada" === line[headerMap.tipo].toLowerCase() ? 1 : 2;
			const conta = await findOrCreateAccount({
				id,
				headerMap,
				accountMap,
				line,
			});
			const categoria = await findOrCreateCategory({
				id,
				headerMap,
				categoryMap,
				line,
			...
			if (!vencimento) throw new Error("Must provide a valid creation date");
...
export const downloadMovimentacoes = async ({
	id,
	conta_id,
	data_inicio,
	data_fim,
	...
	const result = await knex("movimentacao")
		...
		.join(
			"tipo_movimentacao",
			"movimentacao.tipo_movimentacao_id",
		...
		.andWhere({ conta_id })
		.andWhereBetween("vencimento", [
			new Date(data_inicio).toISOString(),
			new Date(data_fim).toISOString(),
		]);
	let csv = "tipo,conta,categoria,vencimento,efetivada,valor,descrição\n";
	result.forEach(
		(r) =>
			(csv += `${r.tipo},${r.conta},${r.categoria},${r.vencimento},${r.efetivada},${r.valor},${r.descricao}\n`),
	);
	return csv;
};

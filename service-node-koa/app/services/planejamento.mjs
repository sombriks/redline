...
export const listPlanejamento = ({
	...
	return knex("planejamento")
		.whereIn(
			"categoria_id",
			knex("categoria").select("id").where({ usuario_id }),
		)
...
export const insertPlanejamento = async ({ usuario_id, planejamento }) => {
	const count = await knex("categoria")
		.where({ usuario_id, id: planejamento.categoria_id })
		.count();
	if (!count[Object.keys(count)[0]])
		throw Error("Categoria não pertence ao usuário");

...
describe('Movimentacao service test', () => {
  ...
  it('Should list movimentacoes by tipo_movimentacao', async () => {
    const usuario = await getAdmin()
    const [conta] = await listContas({ usuario_id: usuario.id })
    const movimentacoes = await listMovimentacaoByConta({ conta_id: conta.id, tipo_movimentacao_id: 2 })
    movimentacoes.should.be.an('array')
    movimentacoes.length.should.be.eql(1)
    expect(movimentacoes.find(m => m.valor == 50)).to.be.ok
  })

  it('Should list movimentacoes by categoria', async () => {
    const usuario = await getAdmin()
    const [conta] = await listContas({ usuario_id: usuario.id })
    const [categoria] = await listCategorias({ usuario_id: usuario.id, q: 'OUTROS' })
    const movimentacoes = await listMovimentacaoByConta({ conta_id: conta.id, categoria_id: categoria.id })
    movimentacoes.should.be.an('array')
    movimentacoes.length.should.be.eql(2)
    expect(movimentacoes.find(m => m.valor == 100)).to.be.ok
  })

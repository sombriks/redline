...
describe("Movimentacao service test", () => {
	...
	it("Should list movimentacoes by usuario", async () => {
		...
		const movimentacoes = await listMovimentacaoByUsuario({
	...
	it("Should list movimentacoes by tipo_movimentacao", async () => {
		const usuario = await getAdmin();
		const [conta] = await listContas({ usuario_id: usuario.id });
		const movimentacoes = await listMovimentacaoByConta({
			conta_id: conta.id,
	...
	it("Should list movimentacoes by categoria", async () => {
		const usuario = await getAdmin();
		const [conta] = await listContas({ usuario_id: usuario.id });
		const [categoria] = await listCategorias({
			usuario_id: usuario.id,
			q: "OUTROS",
		});
		const movimentacoes = await listMovimentacaoByConta({
	...
	it("Should save a new movimentacao", async () => {
		...
		const movimentacao = movimentacoes.find(
			(m) => m.descricao === "teste dinheiro 2",
		);

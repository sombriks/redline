...
export const listMovimentacaoRequest = async ctx => {
  ...
  const {q = "", limit = 50, offset = 0, conta_id, tipo_movimentacao_id} = ctx.request.query;
  if (conta_id) ctx.body = await listMovimentacaoByConta({conta_id, tipo_movimentacao_id, q, limit, offset});
  else ctx.body = await listMovimentacaoByUsuario({usuario_id, tipo_movimentacao_id, q, limit, offset});
};
